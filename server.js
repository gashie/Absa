...
app.listen(PORT, () => {
    console.log(
        `ABSA API: Running in ${process.env.NODE_ENV} mode and listening on port http://:${PORT}`
    );
    logger.debug(`ABSA API: Running in ${process.env.NODE_ENV} mode and listening on port http://:${PORT}`);
});
